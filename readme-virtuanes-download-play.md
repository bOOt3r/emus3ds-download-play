# VirtuaNES for 3DS — Local Multiplayer Fork

**Work in progress.** This fork adds native local multiplayer to the VirtuaNES core in `bubble2k16/emus3ds`. The goal is to let two 3DS/2DS systems play the same two-player NES game together over local wireless while each console runs the NES emulator locally. 

**But why?** For the first time in years i was recommended a video from AVGN, actually it was this video: [AVGN: Bubble Bobble](https://www.youtube.com/watch?v=1mVmTa3KefQ).
...
The emulator now has a **Local Play** flow with three modes:

- **Single** — normal standalone VirtuaNES behavior.
- **Host** — select a ROM, create a local wireless lobby, and act as Player 1.
- **Join** — discover nearby VirtuaNES lobbies, join one, receive the selected ROM for the session, and act as Player 2.

The current implementation has been tested on real hardware and includes:

- native Nintendo 3DS **UDS local wireless** networking;
- nearby-lobby discovery with host/game information;
- host-selected ROM transfer to the guest;
- chunked transfer with acknowledgements and CRC32 verification;
- a temporary session ROM on the guest, so both systems run the exact same ROM image;
- a host-created **canonical VirtuaNES savestate** transferred to the guest before gameplay starts;
- guest-side CRC verification and savestate loading before the synchronized `GO` event;
- deterministic, host-authoritative frame/input synchronization;
- Player 1 input from the host and Player 2 input from the guest;
- connection-state checks so a disconnected peer no longer leaves the emulator stuck forever;
- experimental buffered input delivery to reduce frame drops caused by wireless jitter.

This is **not screen streaming**. Both systems emulate the NES locally. The network is used to establish the same starting state and then keep controller input/frame progression synchronized.
...
Host selects ROM
      │
      ▼
Host advertises Local Play lobby
      │
      ▼
Guest discovers and joins lobby
      │
      ▼
Host transfers ROM ──► Guest CRC32 verifies ROM
      │
      ▼
Host loads ROM and creates canonical VirtuaNES savestate
      │
      ▼
Host transfers state ──► Guest CRC32 verifies + loads state
      │
      ▼
Guest reports STATE_READY
      │
      ▼
Host sends GO
      │
      ▼
Both systems enter the same emulated timeline
      │
      ▼
Host-authoritative synchronized P1 + P2 input
```

The canonical-state step was important. Simply loading/resetting the same ROM independently on both systems was not enough to guarantee identical emulator state because cartridge RAM/save context and other internal emulator state can differ. Copying the host's savestate to the guest gives both emulator instances the same starting point before lockstep begins.

## Inspiration and design direction

### VirtuaNES for 3DS

This project builds on the existing VirtuaNES 3DS port and keeps its emulator/UI structure rather than creating a new NES emulator from scratch:

- https://github.com/bubble2k16/emus3ds

### nesDS

A major architectural reference has been **nesDS** and its local wireless/NiFi multiplayer work:

- https://github.com/DS-Homebrew/NesDS

The important idea we took from studying nesDS is that local multiplayer does not need to stream one console's screen to another. Both systems can emulate the game locally while the multiplayer layer keeps their frame/input timeline synchronized. That is the direction used here as well.

### devkitPro / libctru UDS

The transport layer uses the Nintendo 3DS UDS service exposed by **libctru**. The libctru/3ds-examples projects provide the low-level building blocks for local wireless communication:

- https://github.com/devkitPro/libctru
- https://github.com/devkitPro/3ds-examples

Before integrating networking into VirtuaNES, a small standalone UDS proof-of-concept was used to verify host/join behavior and controller-data transfer on real 3DS hardware.

### Nintendo-style Local Play / Download Play UX

The longer-term UX direction is inspired by Nintendo's local multiplayer flow: one system hosts, nearby systems discover the game, join, and start together with minimal setup. **Nintendo Download Play itself is not implemented yet.** The current guest still runs this VirtuaNES build and receives the ROM/session state over our own UDS protocol.

## Current focus / known limitations

This is still an experimental multiplayer implementation. Current work is focused on:

- maintaining full-speed gameplay while preserving deterministic synchronization;
- absorbing short UDS latency/jitter without causing emulator frame drops;
- improving the disconnect UX so loss of a peer returns cleanly to a Local Play/menu state instead of silently becoming a standalone session;
- long-duration synchronization testing across different NES games and mappers;
- validating special mapper state. The upstream 3DS savestate serializer has a known TODO around `XRAM`, which may matter for some mapper-specific games;
- keeping local-play-specific state isolated from normal single-player save/config behavior.

At the moment the multiplayer design targets **two systems / two NES players**.

## Building this fork

The VirtuaNES target is currently built with:
...
The `.3dsx` build is the primary development target for this work.

---

# Upstream README

The remainder below is the original VirtuaNES-for-3DS README kept for upstream usage, emulator features, history, and credits.
